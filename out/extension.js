...
async function handleEncodingConversion(target, sourceEncoding = 'auto') {
    ...
    // 檢查檔案是否有未儲存的修改
    if (document.isDirty) {
        const choice = await vscode.window.showWarningMessage('檔案有未儲存的修改，編碼轉換會覆寫這些修改。要繼續嗎？', '儲存並轉換', '取消');
        if (choice === '儲存並轉換') {
            await document.save();
        }
        else {
...
async function compileCurrentFile() {
    ...
    if (useCustom && customCommand) {
        ...
        const extraFlags = config.get('compilerFlags', '-Wall -O2');
...
async function runCurrentFile(skipTimeCheck = false) {
    ...
    // 準備執行命令
    let execCommand;
    let useCustomCommand = config.get('useCustomCommand', false);
