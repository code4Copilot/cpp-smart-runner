...
suite('Custom Command Fallback Test Suite', () => {
    // 模擬命令建構邏輯
    function buildCompileCommand(useCustom, customCommand, languageId, sourceFile) {
    ...
    function buildRunCommand(useCustom, customCommand, outputFile) {
        if (useCustom && customCommand) {
            // 使用自訂命令
    ...
    test('Should use default run command when useCustomCommand is false', () => {
        ...
        const isWindows = process.platform === 'win32';
        if (isWindows) {
            assert.ok(command.includes('.\\test.exe') || command === '.\\test.exe', 'Should use Windows relative path format');
        }
        else {
            assert.ok(command.includes('./test.exe') || command === './test.exe', 'Should use Unix relative path format');
        }
    });
    test('Should fallback to default when useCustomCommand is true but customRunCommand is empty', () => {
        ...
        const isWindows = process.platform === 'win32';
        if (isWindows) {
            assert.ok(command.includes('.\\program.exe') || command === '.\\program.exe', 'Should fallback to Windows relative path');
        }
        else {
            assert.ok(command.includes('./program.exe') || command === './program.exe', 'Should fallback to Unix relative path');
        }
    });
    ...
    test('Should handle file names with spaces in default run command', () => {
        ...
        const isWindows = process.platform === 'win32';
        if (isWindows) {
            assert.ok(command.includes('".\\my program.exe"'), 'Should quote Windows path with spaces');
        }
        else {
            assert.ok(command.includes('"./my program.exe"'), 'Should quote Unix path with spaces');
        }
    });
    ...
    test('Configuration: useCustomCommand should default to false', () => {
        const config = vscode.workspace.getConfiguration('cpp-smart-runner');
        const useCustomCommand = config.get('useCustomCommand', false);
        // 應該是 boolean 類型
        assert.strictEqual(typeof useCustomCommand, 'boolean', 'useCustomCommand should be a boolean');
    });
    test('Configuration: customCompileCommand should accept empty string', async () => {
        const config = vscode.workspace.getConfiguration('cpp-smart-runner');
        // 儲存原始值
        const originalValue = config.get('customCompileCommand');
        try {
            // 設定為空字串
            await config.update('customCompileCommand', '', vscode.ConfigurationTarget.Global);
            const updatedConfig = vscode.workspace.getConfiguration('cpp-smart-runner');
            const customCommand = updatedConfig.get('customCompileCommand', '');
            // 應該能夠設定為空字串
            assert.strictEqual(typeof customCommand, 'string', 'customCompileCommand should be a string');
        }
        finally {
            // 恢復原始值
    ...
    test('Configuration: customRunCommand should accept empty string', async () => {
        const config = vscode.workspace.getConfiguration('cpp-smart-runner');
        // 儲存原始值
        const originalValue = config.get('customRunCommand');
        try {
            // 設定為空字串
            await config.update('customRunCommand', '', vscode.ConfigurationTarget.Global);
            const updatedConfig = vscode.workspace.getConfiguration('cpp-smart-runner');
            const customCommand = updatedConfig.get('customRunCommand', '');
            // 應該能夠設定為空字串
            assert.strictEqual(typeof customCommand, 'string', 'customRunCommand should be a string');
        }
        finally {
            // 恢復原始值
    ...
    test('Should not show error when useCustomCommand is true but custom commands are empty', () => {
        ...
        assert.strictEqual(errorThrown, false, 'Should not throw error when falling back to default commands');
    });
});
