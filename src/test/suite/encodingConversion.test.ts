...
suite('編碼轉換功能測試', () => {
    ...
    test('自動編碼轉換應使用可撤銷的編輯器操作', async function() {
        ...
        try {
            ...
            // 檢查檔案在編輯器中可讀（不是亂碼）
            assert.ok(
                contentBefore.includes('stdio.h') || contentBefore.includes('main'),
                '檔案應該可讀（已自動轉換或原本就是 UTF-8）'
            );
...
suite('Big5 轉換單向寫入測試', () => {
    ...
    test('用戶取消未儲存警告時，檔案應保持開啟', async function() {
        ...
        try {
            ...
            // 驗證：檔案內容未改變
            assert.ok(
                activeEditor?.document.getText().includes('// Modified'),
                '修改的內容應該保留'
            );
        } finally {
            if (fs.existsSync(testFilePath)) {
    ...
    test('成功轉換時應寫入 Big5 並關閉編輯器', async function() {
        ...
        try {
            ...
            warningStub.resolves('確定轉換' as any);
            ...
            assert.ok(
                decodedContent.includes('測試中文') || decodedContent.includes('stdio.h'),
                '檔案應該能用 Big5 解碼'
            );
            ...
            assert.ok(
                infoMessage && infoMessage.includes('成功轉換'),
                '訊息應該包含「成功轉換」'
            );
