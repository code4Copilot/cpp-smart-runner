...
- **自訂命令回退邏輯修正** - 改善 Dev-C++ 用戶體驗
  - 修正當 `useCustomCommand` 啟用但 `customCompileCommand` 為空時出現錯誤的問題
  - 修正當 `useCustomCommand` 啟用但 `customRunCommand` 為空時出現錯誤的問題
  - 現在未設定自訂命令時會靜默回退到預設命令，不再顯示錯誤訊息
  - 自訂命令設計為進階功能，不設定時自動使用預設的 gcc/g++ 編譯
  - 對從 Dev-C++ 轉移過來的用戶更友善，無需任何額外設定即可使用

### 測試 🧪
- **新增自訂命令回退測試套件** - 12 個新測試
  - 未啟用自訂命令時使用預設命令測試
  - 啟用但未設定時回退到預設命令測試
  - 啟用且設定時使用自訂命令測試
  - 處理檔案名稱中的空格測試
  - 跨平台支援（Windows/Unix）測試
  - 配置可設為空字串測試
  - 回退時不拋出錯誤測試
  - 總測試數量：138 個測試（全部通過）
...
- **統一執行命令格式** - 解決 PowerShell 執行路徑問題
  ...
  - UTF-8 編碼設定仍然根據終端機類型（PowerShell/CMD）使用不同語法

### 改進 ✨
- **新增語言標準設定選項** - 提升彈性和相容性
  - C++ 預設標準從 `-std=c++17` 改為 `-std=c++11`（支援 Dev-C++ 的 g++）
  - 新增 `cpp-smart-runner.cStandard` 設定（預設：`-std=c11`）
  - 新增 `cpp-smart-runner.cppStandard` 設定（預設：`-std=c++11`）
  - 支援彈性設定：C99/C11/C17/C2x 和 C++11/C++14/C++17/C++20/C++23
  - 使用者可輕鬆修改語言標準，無需完全自訂編譯命令
  - 保持自動判斷 C/C++ 檔案類型的功能

- **簡化程式碼邏輯**
  - 移除複雜的終端機類型判斷邏輯（針對執行命令）
  - 採用統一格式，降低維護成本
  - 保留 UTF-8 編碼設定的終端機類型區分（因語法不同）
